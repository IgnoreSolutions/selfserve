...
router.post('/tokencheck', (req, res) => {
    if (req.body.username != null && req.body.token != null) {
        ...
        const fs = require('fs');
        if (fs.existsSync(`./user_tokens/${username}`)) {
            fs.readFile(`./user_tokens/${username}`, { encoding: 'utf-8' }, (err, data) => {
                if (!err) {
                    if (data.length > 1) {
                        if (data === req.body.token) {
                            ...
                            res.status(401).send(`bad token ${data} vs ${req.body.token}`);
                        }
                    }
                }
                else {
                    res.status(401).send("couldn't load token from disk " + err);
                }
            });
        }
    }
